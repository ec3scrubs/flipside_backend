...
def execute_queries(input_post_file, query, dictionary, document_length):
    """
    ...
    stars = get_stars(top_10) # returns a dictionary
    return stars
...
# Quick hack to get the stars of each review, should have done this indexing but I'm lazy
def get_stars(doc_list):
    stars = {}
    for doc in doc_list:
        f = file("dl/output/" + str(doc), "r")
...
# Execution
def main(dictionary, posting, query):
    input_dict_file = dictionary
    input_post_file = posting
    (dictionary, document_length) = build_dict(input_dict_file)
    return execute_queries(input_post_file, query, dictionary, document_length)
